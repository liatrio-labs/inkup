...
fn notice(latest: &Version, install: &Install, choice: Option<HomebrewChoice>, skew: Option<&Skew>) -> String {
    ...
    match skew {
        ...
        Some(skew) => format!("inkup {latest} is out{} then {run}", skew.clause()),
    }
}

/// Starts the daily check in the background and returns at once (`inkup_update_check::spawn_check`): only at a
/// terminal. The receiver holds the TUI's notice once a newer release is known.
pub fn spawn_check(dir: &Path) -> watch::Receiver<Option<String>> {
    let allowed = check_allowed(std::io::stdout().is_terminal(), |name| std::env::var(name).ok());
    inkup_update_check::spawn_check(dir, allowed, |dir, found: &Found| {
        let choice = load_choice(dir).ok().flatten();
        notice(&found.latest, &detect(), choice, found.skew.as_ref())
    })
}

...
4. `.github/workflows/inkup-v-release.yml` then:
   ...
   - then calls `.github/workflows/desktop-macos.yml`, which waits for approval in the `release` environment and
     attaches the desktop app's DMG ([The desktop app's DMG](#the-desktop-apps-dmg)).
...
re-run the failed job: the upload replaces any earlier DMG.
...
commit. The release is a pre-release, so Homebrew skips it, but `desktop-macos` still runs. Approve it, and when it is
green:

...
**Proving it with a pre-release.** dist refuses a tag whose version is not the `inkup` crate's. So bump
`host/Cargo.toml` to the pre-release first, e.g. `0.2.0-rc.1`, merge that, then tag `inkup-v0.2.0-rc.1` on the merge
commit. The release is a pre-release, so Homebrew skips it (the formula and the cask), but `desktop-macos` still
runs. Approve it, and when it is
green:
